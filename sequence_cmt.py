...
from metrics import accuracy, kappa, g_mean, plot_confusion_matrix, confusion_matrix, AverageMeter
from interpret import plot_interpret, softmax, scaled_dot_product_attention_mod
...
def train_seq_cmt(Net, train_data_loader, val_data_loader, criterion, optimizer, lr_scheduler, device, args):
    ...
    for epoch_idx in range(args.n_epochs):  # loop over the dataset multiple times
        ...
        Net.train()
        print(
            f'============================= Training Epoch : [{epoch_idx + 1}/{args.n_epochs}]===============================>')
        ...
        with torch.no_grad():
            ...
            for batch_val_idx, data_val in enumerate(val_data_loader):
                ...
                for ep in range(args.num_seq):
                    ...
                    sens_list, spec_list, f1_list, prec_list, sens, spec, f1, prec = confusion_matrix(pred[ep].cpu(),
                                                                                                      val_labels[:, ep],
                                                                                                      5,
                                                                                                      cur_val_batch_size)
        ...
        print(
            f'===========================================================Epoch : [{epoch_idx + 1}/{args.n_epochs}]  Evaluation ===========================================================================================================>')
        print("Training Results : ")
        print(
            f"Training Loss     : {losses.avg}, Training Accuracy      : {train_accuracy.avg}, Training G-Mean      : {train_gmean.avg}")
        print(
            f"Training Kappa      : {train_kappa.avg},Training MF1     : {train_f1_score.avg}, Training Precision      : {train_precision.avg}, Training Sensitivity      : {train_sensitivity.avg}, Training Specificity      : {train_specificity.avg}")

        print("Validation Results : ")
        print(
            f"Validation Loss   : {val_losses.avg}, Validation Accuracy : {val_accuracy.avg}, Validation G-Mean      : {val_gmean.avg}")
        print(
            f"Validation Kappa     : {val_kappa.avg}, Validation MF1      : {val_f1_score.avg}, Validation Precision      : {val_precision.avg},  Validation Sensitivity      : {val_sensitivity.avg}, Validation Specificity      : {val_specificity.avg}")

        print(
            f"Class wise sensitivity W: {class1_sens.avg}, S1: {class2_sens.avg}, S2: {class3_sens.avg}, S3: {class4_sens.avg}, R: {class5_sens.avg}")
        print(
            f"Class wise specificity W: {class1_spec.avg}, S1: {class2_spec.avg}, S2: {class3_spec.avg}, S3: {class4_spec.avg}, R: {class5_spec.avg}")
        print(
            f"Class wise F1  W: {class1_f1.avg}, S1: {class2_f1.avg}, S2: {class3_f1.avg}, S3: {class4_f1.avg}, R: {class5_f1.avg}")
        ...
        if val_accuracy.avg > best_val_acc or (epoch_idx + 1) % 100 == 0 or val_kappa.avg > best_val_kappa:
            ...
            if (epoch_idx + 1) % args.save_model_freq == 0:
                torch.save(Net,
                           f'{args.project_path}/model_check_points/checkpoint_model_epoch_{epoch_idx + 1}.pth.tar')
        lr_scheduler.step()
...
# 更改了代码中的类型不匹配错误
def new_train_seq_cmt(Net, train_data_loader, val_data_loader, criterion, optimizer, lr_scheduler, device, args):
    ...
    for epoch_idx in range(args.n_epochs):  # loop over the dataset multiple times
        ...
        for batch_idx, data_input in enumerate(train_data_loader):
            ...
            ##############################
            # print(type(outputs[ep].cpu(), labels[:,ep]))
            ##############################
            ...
            ##############################
            for ep in range(args.num_seq):
                # print(type(criterion(outputs[ep].cpu(), labels[:,ep])))
                loss += criterion(outputs[ep].cpu(), labels[:,ep].long())
                train_accuracy.update(accuracy(outputs[ep].cpu(), labels[:,ep]))
                _,_,_,_,sens,spec,f1, prec = confusion_matrix(outputs[ep].cpu(), labels[:,ep], 5, cur_batch_size)
                train_sensitivity.update(sens)
                train_specificity.update(spec)
        ...
        with torch.no_grad():
          ...
          for batch_val_idx, data_val in enumerate(val_data_loader):
            ...
            #####################################
            for ep in range(args.num_seq):
                  # print(type(criterion(pred[ep].cpu(), val_labels[:,ep])))
                  val_loss += criterion(pred[ep].cpu(), val_labels[:,ep])
