...
from sleep_edf import split_data, SleepEDF_MultiChan_Dataset, get_dataset, new_get_dataset, new_get_dataset_EEG5_EOG1, new_get_dataset_EEG1_EOG1
from epoch_cmt import Epoch_Cross_Transformer_Network, train_epoch_cmt
from sequence_cmt import Seq_Cross_Transformer_Network #as Seq_Cross_Transformer_Network
from sequence_cmt import train_seq_cmt, new_train_seq_cmt
...
def parse_option():
    ...
    parser.add_argument('--project_path', type=str, default='./results', help='Path to store project results') # 存储项目结果的位置
    parser.add_argument('--data_path', type=str, help='Path to the dataset file') # 经过预处理的数据集地址
    parser.add_argument('--train_data_list', nargs="+", default = [0,1,2,3] ,  help='Folds in the dataset for training') # 训练列表
    parser.add_argument('--val_data_list', nargs="+", default = [4] ,  help='Folds in the dataset for validation') # 评价列表
    parser.add_argument('--is_retrain', type=bool, default=False,   help='To retrain a from saved checkpoint') # 是否用已有的模型继续训练
    parser.add_argument('--model_path', type=str, default="",   help='Path to saved checkpoint for retraining') # 已有模型存放的位置
    parser.add_argument('--save_model_freq', type=int, default = 50 ,  help='Frequency of saving the model checkpoint') # 保存数据集的频率

    #model parameters
    parser.add_argument('--model_type', type=str, default = 'Epoch'  ,choices=['Epoch', 'Seq'],  help='Model type') # 训练的模型类型
...
def main():
    ...
    train_data_loader, val_data_loader = new_get_dataset_EEG1_EOG1(device, args)
    ...
    #############################
    print("args.is_neptune = " + str(args.is_neptune))
    if (args.is_neptune == True):
        args.is_neptune = False
        print("Now args.is_neptune = " + str(args.is_neptune))
    #############################
    ...
    if args.model_type == "Seq":
        new_train_seq_cmt(Net, train_data_loader, val_data_loader, criterion, optimizer, lr_scheduler, device, args)
